    ...
    public class OrderRepository : GeneralRepository<Order>
    {
...

        {
            ...
            if (registerDate != null)
                filter = filter.Or(o => o.Date.Year == registerDate.Value.Year);

            if (filter == oldFiler)
                filter = null;
            var query = base.Get(filter, orderBy, isAscending, pageIndex, pageSize, "OrderItems");
            ...
            query.Select(i => new OrderViewModel
            {
                ...
                Date = i.Date,
                OrderItems = i.OrderItems.Select(o => o.ToViewModel()).ToList()
